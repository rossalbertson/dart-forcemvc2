...
class ServingFiles {
  ...
  void _serveClient(staticFiles, clientFiles, clientServe) {
    if (clientServe == true) {
      ...
      virDir =
          new http_server.VirtualDirectory(clientFilesAbsoluteUri.toFilePath());
      ...
      log.info(
          "You are serving the clientside files your self! Force is not serving clientside files!");
    }
  }
  ...
  void _serveStaticFiles(clientFiles) {
    ...
    router.serve(pattern).listen((request) {
      ...
      if (servingAssistent != null) {
        servingAssistent
            .serveFromFile(request, "${clientFiles}${path}")
            .catchError((e) {
          print(e);
          _notFoundHandling(request);
        });
